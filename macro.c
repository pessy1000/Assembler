...
FILE * macros (FILE * ifp, char * file_name)
{
	...
	for(a = 0; a < num_of_mcr; a++)/*check that all macro names are legal*/
	{
		for(b = a + 1; b < num_of_mcr; b++)
		{
			if(strcmp(mcr_arr[a].name, mcr_arr[b].name) == 0 && a != b)/*same name not used twice*/
			{
		...
		if((strcmp(mcr_arr[a].name, "extern") == 0) || (strcmp(mcr_arr[a].name, "entry") == 0) ||
		(strcmp(mcr_arr[a].name, "data") == 0) || (strcmp(mcr_arr[a].name, "string") == 0) || 
		(strcmp(mcr_arr[a].name, "mcr") == 0) || (strcmp(mcr_arr[a].name, "endmcr") == 0))
		/*If the label is the name of a reserved word we will return an error*/
		{
			fprintf(stderr, "ERROR - illegal name '%s' - for macro\n",mcr_arr[a].name);
			error = 1;
		}
		if((strcmp(mcr_arr[a].name, "r0") == 0) || (strcmp(mcr_arr[a].name, "r1") == 0) || (strcmp(mcr_arr[a].name, "r2") == 0) ||
		(strcmp(mcr_arr[a].name, "r3") == 0) || (strcmp(mcr_arr[a].name, "r4") == 0) || (strcmp(mcr_arr[a].name, "r5") == 0) ||
		(strcmp(mcr_arr[a].name, "r6") == 0) || (strcmp(mcr_arr[a].name, "r7") == 0))
		/*if macro is name of register */
		{
			fprintf(stderr, "ERROR - illegal name '%s' - for macro\n",mcr_arr[a].name);
			error = 1;
		}
