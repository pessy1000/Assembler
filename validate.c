...
void is_valid_extern(char * str, int line_num,FILE *ifp)
{
	...
	word = strtok(str,",\n ");
	if (word == NULL)/*if too many operands*/ 
	{
	...
	while (fgets(line, sizeof(line), ifp)) 
	{
		...
		for(i = 0; i < 16; i++)
		{
			if(strstr(line, opcode[i].opName))
			{
				...
				strncpy(opcode_line, &(opcode_ptr[4]), strlen(opcode_ptr) - 5);/*copies to 'opcode line' from after opcode name*/
